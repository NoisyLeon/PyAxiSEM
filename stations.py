...
class StaInfo(object):
    ...
    ========================================================================
    """
    def __init__(self, stacode=None, network='AXI',  lat=None, lon=None, elevation=0.0, depth=0.0):
...
class StaLst(object):
    ...
    def write(self, outdir):
        ...
        #     os.makedirs(outdir)
        outfname=outdir+'/STATIONS'
        with open(outfname,'wb') as f:
            for station in self.stations:
                tempstr='%s\t %s\t %f\t %f\t %f\t %f\t \n' % (station.stacode, station.network,
                                                        station.lat, station.lon, station.elevation, station.depth)
                f.writelines(tempstr)
                # f.writelines('%2.6f %3.6f 0.0\n' %(90.-station.lat, station.lon) )
        return
    # 
    def homo_stalst(self, minlat, Nlat, minlon, Nlon, dlat, dlon, net='AXI', prx=''):
        """
        ...
        for ilon in xrange(Nlon):
            for ilat in xrange(Nlat):
                ...
                elevation=0.0
                stacode=prx+str(ilat)+'S'+str(ilon)
