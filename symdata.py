...
class axisemtrace(obspy.core.trace.Trace):
    """
    saxisemtrace:
...
class axisem_seismogram(object):
    """An object to handle txt seismogram files from AXISEM
    ===================================================================================
    Parameters:
    nt          - NPTS
    dt          - time interval
    ...
    def __init__(self):
        ...
        # self.sy=0.0
        # self.sz=0.0
        self.trace_E= np.array([])
        self.trace_N= np.array([])
...
class axisemASDF(pyasdf.ASDFDataSet):
    
    def readtxt(self, workingdir=None, stafile=None,  datadir=None, channel='all', verbose=True, delta=None):
        ...
        ===================================================================================
        """
        if workingdir==None and ( datadir==None or stafile==None):
            raise ValueError('Specify workingdir or datadir/stafile!')
        if workingdir!=None:
            stafile = workingdir + '/STATIONS'
            datadir = workingdir + '/Data_Postprocessing/SEISMOGRAMS'
        ...
        for sta in SLst.stations:
            fprx = datadir +'/'+sta.stacode+'_'+sta.network+'_*'
            print fprx
            if channel == 'all':
                st = obspy.Stream()
                for chan in chanlst:
                    fnameLst = glob.glob(fprx+chan+'.dat')
                    if len(fnameLst) ==0:
                        print 'WARNING: no seismogram for', fprx+chan+'.dat'
                        continue
                    if len(fnameLst) >1:
                        print 'WARNING: more than one seismogram for', fprx+chan+'.dat'
                        continue
                    inArr   = np.loadtxt(fnameLst[0])
                    time    = inArr[:,0]; sig   = inArr[:,1]
                    tr      = obspy.Trace()
                    tr.data = sig
                    tr.stats.delta= time[1] - time[0]
                    tr.stats.network = sta.network
                    tr.stats.station = sta.stacode
                    tr.stats.channel = 'BX'+chan
                    st.append(tr)
                if delta !=None: st.resample(sampling_rate=1./delta)
                self.add_waveforms( st , tag='axisem_raw')
            else:
                fnameLst = glob.glob(fprx+channel+'.dat')
                if len(fnameLst) ==0:
                    print 'WARNING: no seismogram for', fprx+channel+'.dat'
                    continue
                if len(fnameLst) >1:
                    print 'WARNING: more than one seismogram for', fprx+channel+'.dat'
                    continue
                inArr   = np.loadtxt(fnameLst[0])
                time    = inArr[:,0]; sig   = inArr[:,1]
                tr      = obspy.Trace()
                ...
                tr.stats.station = sta.stacode
                tr.stats.channel = 'BX'+chan
                if delta !=None: tr.resample(sampling_rate=1./delta)
                self.add_waveforms( tr , tag='axisem_raw')
        print 'End reading txt files!'
        return
            
    
    def read_event(self, infname):
        with open(infname, 'r') as f:
            for line in f.readlines():
                lsplit = line.split()
                if len(lsplit) == 0: continue
                if lsplit[0] == 'SOURCE_LAT': lat = float(lsplit[1])
                elif lsplit[0] == 'SOURCE_LON': lon = float(lsplit[1])
                elif lsplit[0] == 'SOURCE_DEPTH': depth = float(lsplit[1])
                elif lsplit[0] == 'latitude:': lat = float(lsplit[1])
                elif lsplit[0] == 'longitude:': lon = float(lsplit[1])
                elif lsplit[0] == 'depth:': depth = float(lsplit[1])
                else: continue
        self.add_event(evlo=lon, evla=lat, evdp=depth)
        return
                
    
    def add_event(self, evlo, evla, evdp):
    ...
    def write2sac(self, staid=None, lon=None, lat=None, outdir='.', SLst=None, channel='BXZ'):
        ...
        if staid!=None:
            tr=self.waveforms[staid].axisem_raw.select(channel=channel)[0]
            ...
            for staid in self.waveforms.list():
                ...
                if stlo!=lon or stla!=lat:
                    ...
                    tr=subdset.axisem_raw.select(channel=channel)[0]
                    break
        ...
        sacTr.stlo  = lon
        sacTr.stla  = lat
        outsacfname=outdir+'/'+staid+'_'+channel+'.SAC'
        sacTr.write(outsacfname)
        return
    
    def get_trace(self, staid=None, lon=None, lat=None, SLst=None, channel='BXZ'):
        if staid==None and (lon==None or lat==None):
            raise ValueError('Error Input')
        evlo    = self.events.events[0].origins[0].longitude
        evla    = self.events.events[0].origins[0].latitude
    ...
    def get_stream(self, staid=None, lon=None, lat=None, SLst=None):
        if staid==None and (lon==None or lat==None):
            raise ValueError('Error Input')
        evlo    = self.events.events[0].origins[0].longitude
        evla    = self.events.events[0].origins[0].latitude
        if staid    !=  None:
            st              = self.waveforms[staid].axisem_raw
            lat, elev, lon  = self.waveforms[staid].coordinates.values()
        elif isinstance(SLst, stations.StaLst):
            for sta in SLst.stations:
                if sta.lon!=lon or sta.lat!=lat:
            ...
            for staid in self.waveforms.list():
                ...
                if stlo!=lon or stla!=lat:
                    ...
                    st  = subdset.axisem_raw
                    break
        print evla, evlo, lat, lon
        dist, az, baz=obspy.geodetics.gps2dist_azimuth(evla, evlo, lat, lon) # distance is in m
